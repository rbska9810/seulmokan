...
const notes={
  ...
  'bmi-calculator':'<h2>결과 해석 주의</h2><p>BMI는 키와 체중만 이용한 선별 지표입니다. 연령, 근육량, 임신 여부와 개인 건강 상태를 반영하지 않으며 의료 진단으로 사용할 수 없습니다.</p>',
  'calorie-calculator':'<h2>결과 해석 주의</h2><p>기초대사량 공식과 활동계수를 이용한 추정치입니다. 실제 필요 열량은 신체 구성, 질환, 운동 강도와 생활환경에 따라 달라집니다.</p>',
  ...
  'svg-optimizer':'<h2>지원 범위</h2><p>주석과 태그 사이 공백을 정리하는 간이 최적화입니다. 복잡한 필터, 애니메이션 또는 외부 참조가 포함된 SVG는 원본을 보관하고 결과를 화면에서 확인하세요.</p>',
  'ladder-game':'<h2>추첨 방식</h2><p>화면에 사다리 선을 그리는 방식 대신 브라우저의 보안 난수로 결과 목록을 섞어 참가자와 일대일로 연결합니다. 동일한 참가 인원과 결과 인원을 사용하는 것이 좋습니다.</p>',
...
'border-radius-generator':['네 모서리 값과 px·% 단위를 선택합니다.','도형 미리보기와 border-radius 코드를 생성합니다.','퍼센트 반지름은 요소의 가로·세로 크기에 따라 모양이 달라집니다.'],
...
'svg-optimizer':['완전한 svg 시작·종료 태그가 있는 XML 코드를 입력합니다.','불필요한 공백을 줄이고 위험 요소를 제거한 코드와 감소율을 표시합니다.','script, foreignObject, 이벤트 속성은 보안을 위해 제거되므로 복잡한 SVG는 원본과 비교하세요.'],
...
'lorem-ipsum-generator':['문단 수, 문장 수와 라틴어·한국어 더미 문장 종류를 선택합니다.','요청한 분량의 복사 가능한 시안용 본문을 생성합니다.','더미 문장은 공개 전 반드시 실제 콘텐츠로 교체하세요.'],
'text-sorter':['여러 행과 정렬 기준, 방향, 중복·빈 줄 처리 옵션을 입력합니다.','가나다·숫자·길이·자연 정렬 기준으로 목록을 재배치합니다.','숫자 정렬에서 숫자가 아닌 행이 섞이면 의도한 순서인지 확인하세요.'],
...
'word-sentence-counter':['분석할 본문을 입력합니다.','문자·단어·문장·문단·한글 음절과 예상 읽기 시간을 계산합니다.','문장부호가 없는 문장과 복합어는 편집기의 집계 방식과 차이가 날 수 있습니다.'],
'text-diff-checker':['원본과 수정본을 각각 입력합니다.','행 단위로 유지·추가·삭제된 부분을 색상과 기호로 구분합니다.','매우 큰 문서는 브라우저 성능을 위해 크기 제한이 적용됩니다.'],
'korean-text-cleaner':['한국어 문장과 공백·문장부호·따옴표 정리 옵션을 선택합니다.','선택한 규칙으로 정돈된 문장과 전후 글자 수를 표시합니다.','이 도구는 맞춤법 사전 검사가 아니라 교정 전 형식 정리 도구입니다.'],
...
'bmi-calculator':['성인의 키와 몸무게를 입력합니다.','BMI 분류와 해당 키의 일반적인 정상 체중 범위를 계산합니다.','근육량, 성장기, 임신과 질환을 반영하지 않으므로 의료 진단으로 쓰지 마세요.'],
'calorie-calculator':['만 나이, 키, 체중, 계산식 성별, 활동 수준과 목표를 입력합니다.','기초대사량, 유지 칼로리와 목표 섭취량을 추정합니다.','활동계수는 평균값이며 질환이나 치료 목적 식단은 전문가와 상의하세요.'],
...
'ladder-game':['참가자와 결과를 같은 개수로 한 줄씩 입력합니다.','보안 난수로 결과를 섞어 참가자와 일대일 대응표를 만듭니다.','추첨 전 목록을 모두에게 확인시키고 결과를 다시 실행하면 달라진다는 점을 공유하세요.'],
'unit-converter':['종류, 숫자 값, 변환 전·후 단위를 선택합니다.','길이·무게·온도·속도·부피를 정밀 환산합니다.','컵과 큰술은 표시된 미터법 용량 기준이며 국가별 조리 단위와 다를 수 있습니다.'],
'random-number-generator':['정수 최솟값·최댓값, 개수와 중복·정렬 여부를 입력합니다.','편향을 줄인 보안 난수로 원하는 개수의 숫자를 생성합니다.','중복을 막으면 범위 안의 정수 개수보다 많이 생성할 수 없습니다.'],
...
'stopwatch':['시작 후 원하는 지점마다 랩 버튼이나 L 키를 누릅니다.','구간 시간과 누적 시간을 표로 기록하고 복사할 수 있습니다.','탭 절전과 기기 성능의 영향을 받으므로 공식 경기 계측용으로 쓰지 마세요.'],
...
function completeMetadata(h){
  const title=h.match(/<title>(.*?)<\/title>/)[1];
  const desc=h.match(/name="description" content="([^"]*)"/)[1];
  const name=title.split(' — ')[0].replace(' | 쓸모칸','');
  for(const [key,value] of [['keywords',name+', 무료 온라인 도구, 쓸모칸'],['google-adsense-account','ca-pub-9462573435168414'],['twitter:title',title],['twitter:description',desc]]){
    if(!h.includes(`name="${key}"`))h=h.replace('</head>',`<meta name="${key}" content="${value}"></head>`);
  }
  return h;
}
for(let i=0;i<tools.length;i++){const [slug,title,cat,desc]=tools[i];const related=tools.filter((x,j)=>x[2]===cat&&j!==i).slice(0,3);const url=base+'tools/'+slug+'/';const note=notes[slug]||'',guide=guideDetails[slug];if(!guide)throw Error(`${slug} 상세 안내 없음`);const html=`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)} — 무료 온라인 도구 | 쓸모칸</title><meta name="description" content="${esc(desc)} 설치와 로그인 없이 무료로 사용할 수 있습니다."><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${url}"><link rel="icon" href="../../assets/favicon.svg" type="image/svg+xml"><meta property="og:type" content="website"><meta property="og:locale" content="ko_KR"><meta property="og:site_name" content="쓸모칸"><meta property="og:title" content="${esc(title)} | 쓸모칸"><meta property="og:description" content="${esc(desc)}"><meta property="og:url" content="${url}"><meta property="og:image" content="${base}assets/og-image.png"><meta property="og:image:width" content="1200"><meta property="og:image:height" content="630"><meta property="og:image:alt" content="쓸모칸 로고"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:image" content="${base}assets/og-image.png"><link rel="preconnect" href="https://cdn.jsdelivr.net" crossorigin><link rel="stylesheet" crossorigin href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/variable/pretendardvariable-dynamic-subset.min.css"><link rel="stylesheet" href="../../assets/site.css"><link rel="stylesheet" href="../../assets/tool-ui.css"><script type="application/ld+json">${JSON.stringify(schemaFor(title,url,desc,cat,guide))}</script></head><body>${header}<main class="wrap"><div class="crumb"><a href="../../">홈</a> / ${esc(cat)} / ${esc(title)}</div><section class="hero"><div class="eyebrow">${esc(cat)}</div><h1>${esc(title)}</h1><p class="lead">${esc(desc)}</p><div class="badges"><span class="badge">무료</span><span class="badge">로그인 없음</span><span class="badge">모바일 지원</span><span class="badge">브라우저 처리</span></div></section><div class="layout"><div><section class="panel" id="tool" data-slug="${slug}" aria-label="${esc(title)} 실행 영역"><p>도구를 불러오는 중입니다.</p></section><aside class="ad" aria-label="광고">ADVERTISEMENT<br>반응형 광고 영역</aside><article class="content-card tool-guide" data-guide="complete"><h2>${esc(title)} 사용 방법</h2><ol><li><b>입력값 준비:</b> ${esc(guide[0])}</li><li><b>실행과 확인:</b> 필요한 옵션을 고른 다음 계산·변환·생성 버튼을 누릅니다. 잘못된 값은 결과 영역에 수정 방법과 함께 표시됩니다.</li><li><b>결과 활용:</b> ${esc(guide[1])}</li></ol><h2>입력값과 결과 읽는 법</h2><p>${esc(desc)} 입력 범위가 있는 숫자와 날짜는 범위를 벗어나면 계산하지 않으며, 결과는 한국어 숫자 표기와 단위를 함께 표시합니다. 초기화 버튼으로 예시값을 복원하고 Ctrl 또는 ⌘ + Enter로 빠르게 실행할 수 있습니다.</p><h2>사용 전 확인사항</h2><p>${esc(guide[2])}</p><h2>개인정보와 처리 방식</h2><p>대부분의 계산과 변환은 현재 브라우저에서 실행되어 입력값을 쓸모칸 서버에 저장하지 않습니다. 외부 연결이 필요한 기능은 실행 영역과 개인정보처리방침에 전송 대상과 목적을 표시합니다.</p>${note}<h2>자주 묻는 질문</h2><h3>${esc(title)}는 무료인가요?</h3><p>네. 회원가입과 설치 없이 무료로 사용할 수 있습니다.</p><h3>어떤 값을 입력해야 하나요?</h3><p>${esc(guide[0])}</p><h3>결과가 공식 증빙으로 사용되나요?</h3><p>일반 변환값은 바로 활용할 수 있지만 급여·금융·건강·근로 관련 수치는 참고용입니다. 중요한 결정 전에는 최신 공식 자료와 담당 기관의 결과를 확인하세요.</p></article><section class="content-card"><h2>관련 도구</h2><div class="related">${related.map(x=>`<a href="../${x[0]}/">${esc(x[1])} →</a>`).join('')}</div></section></div><aside class="side"><div class="ad">ADVERTISEMENT<br>300 × 400</div></aside></div></main>${footer}<script src="../../assets/common.js"></script><script src="../../assets/tools.js"></script></body></html>`;const dir=path.join(root,'tools',slug);fs.mkdirSync(dir,{recursive:true});fs.writeFileSync(path.join(dir,'index.html'),completeMetadata(html));}
const infoPages={guide:['사용가이드','쓸모칸의 도구를 빠르고 안전하게 사용하는 방법을 안내합니다.','홈에서 검색창이나 카테고리 필터로 필요한 도구를 찾고, 도구 페이지의 입력란에 값을 넣은 다음 실행 버튼을 누르세요. 결과 복사가 제공되는 도구는 버튼 한 번으로 클립보드에 저장할 수 있습니다.<h2>도구별 확인 사항</h2><p>텍스트·개발 도구는 대부분 브라우저 안에서 처리됩니다. 날짜와 단위는 입력 형식을 확인하고, 생성 결과는 복사 전에 한 번 검토하세요.</p><h2>중요한 계산 결과</h2><p>급여·금융·건강 계산은 참고값입니다. 세금과 보험료는 적용 연도 및 개인 조건에 따라 달라지므로 중요한 결정 전에는 공식 기준을 다시 확인하세요.</p><h2>외부 서비스 사용</h2><p>내 IP 확인과 QR코드 생성기는 기능 수행을 위해 외부 서비스에 요청하며 실행 화면에 데이터 전송 사실을 표시합니다.</p>'],about:['쓸모칸 소개','작은 작업 하나가 하루를 무겁게 만들지 않도록, 빠르고 안전한 온라인 도구를 만듭니다.','쓸모칸은 반복되는 계산과 변환을 설치나 회원가입 없이 끝낼 수 있도록 만든 무료 온라인 유틸리티 서비스입니다.<h2>운영 원칙</h2><p>가능한 작업은 사용자의 브라우저 안에서 처리하고, 기능보다 광고가 앞서지 않는 경험을 지향합니다. 작동하지 않는 링크를 공개하지 않고 계산 기준과 제한사항을 함께 설명합니다.</p><h2>콘텐츠 관리</h2><p>도구의 기능, 안내문과 계산 기준은 쓸모칸 운영자가 직접 관리합니다. 오류가 발견되면 문의 페이지를 통해 제보할 수 있습니다.</p>'],privacy:['개인정보처리방침','쓸모칸에서 입력 데이터와 접속 정보가 어떻게 처리되는지 개인정보 보호 원칙을 자세히 안내합니다.','<p><b>시행일: 2026년 9월 9일</b></p><h2>입력 데이터</h2><p>쓸모칸의 일반 도구는 입력값을 브라우저 내부에서 처리하며 별도 서버에 저장하지 않습니다. 내 IP 확인은 ipify, QR코드 생성은 QR Server에 요청하며 해당 화면에서 전송 사실을 안내합니다.</p><h2>접속 기록과 외부 자원</h2><p>사이트는 GitHub Pages에서 제공되며 Pretendard 폰트를 jsDelivr에서 불러옵니다. 각 사업자는 보안과 서비스 제공을 위해 IP 주소, 브라우저 정보와 같은 기술 로그를 처리할 수 있습니다.</p><h2>광고와 쿠키</h2><p>Google AdSense를 적용할 경우 Google과 제3자 광고 사업자가 쿠키 또는 유사 기술을 사용해 광고를 제공하고 성과를 측정할 수 있습니다. 실제 광고 적용 전에 필요한 동의 안내와 거부 방법을 추가합니다.</p><h2>문의 정보</h2><p>이메일 문의 시 발신 주소, 제목과 본문이 Cloudflare Email Routing을 거쳐 운영자의 메일함으로 전달되며 문의 확인과 답변을 위해 보관될 수 있습니다. 민감한 개인정보는 보내지 마세요. 개인정보 관련 요청은 contact@seulmokan.com으로 접수할 수 있습니다.</p>'],terms:['이용약관','무료 온라인 도구인 쓸모칸을 안전하고 올바르게 이용하기 위한 권리와 책임을 안내합니다.','<p><b>시행일: 2026년 9월 9일</b></p><h2>서비스 제공</h2><p>쓸모칸은 회원가입 없이 사용할 수 있는 무료 온라인 도구를 현재 상태로 제공합니다. 기능과 제공 범위는 보안, 정확성 및 운영상 필요에 따라 변경될 수 있습니다.</p><h2>결과의 활용</h2><p>계산 결과는 참고용이며 법률·세무·금융·의료 전문가의 판단을 대신하지 않습니다. 사용자는 중요한 결과를 공식 자료와 대조해야 합니다.</p><h2>사용자의 책임</h2><p>사용자는 본인이 사용할 권한이 있는 데이터만 입력해야 하며 타인의 개인정보와 저작권을 침해해서는 안 됩니다. 결과 사용 전 최종 확인 책임은 사용자에게 있습니다.</p><h2>외부 서비스</h2><p>일부 기능은 외부 서비스의 가용성과 정책에 영향을 받을 수 있습니다. 외부 서비스로 전송되는 정보는 해당 도구 화면에서 안내합니다.</p>'],contact:['문의하기','쓸모칸 도구의 오류를 제보하거나 필요한 새로운 기능과 온라인 도구를 제안하는 방법을 안내합니다.','<h2>문의 방법</h2><p>contact@seulmokan.com으로 이메일을 보내주세요. 아래 버튼을 누르면 기본 메일 앱이 열립니다. 사용한 도구 이름, 문제 상황과 기대한 결과를 함께 적어주시면 확인에 도움이 됩니다.</p><h2>오류 제보에 포함할 내용</h2><p>도구 이름, 사용한 브라우저, 입력값의 형식과 발생한 오류를 알려주면 확인에 도움이 됩니다. 실제 개인정보 대신 재현 가능한 예시 값을 사용하세요.</p><h2>보내면 안 되는 정보</h2><p>비밀번호, 주민등록번호, 계좌·카드정보, 업무상 비밀과 같은 민감한 내용은 보내지 마세요.</p>']};
for(const [slug,[title,lead,body]] of Object.entries(infoPages)){const url=base+slug+'/';const extra=slug==='contact'?`<p><a class="btn" href="mailto:contact@seulmokan.com?subject=%EC%93%B8%EB%AA%A8%EC%B9%B8%20%EB%AC%B8%EC%9D%98">이메일로 문의하기</a></p><p><a href="mailto:contact@seulmokan.com">contact@seulmokan.com</a></p>`:'';const html=`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title} | 쓸모칸</title><meta name="description" content="${lead}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${url}"><link rel="icon" href="../assets/favicon.svg" type="image/svg+xml"><meta property="og:type" content="website"><meta property="og:locale" content="ko_KR"><meta property="og:site_name" content="쓸모칸"><meta property="og:title" content="${title} | 쓸모칸"><meta property="og:description" content="${lead}"><meta property="og:url" content="${url}"><meta property="og:image" content="${base}assets/og-image.png"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:image" content="${base}assets/og-image.png"><link rel="stylesheet" crossorigin href="https://cdn.jsdelivr.net/gh/orioncactus/pretendard@v1.3.9/dist/web/variable/pretendardvariable-dynamic-subset.min.css"><link rel="stylesheet" href="../assets/site.css"><script type="application/ld+json">${JSON.stringify({'@context':'https://schema.org','@type':'WebPage',name:title,url,description:lead,inLanguage:'ko-KR'})}</script></head><body>${header.replaceAll('../../','../')}<main class="wrap"><div class="crumb"><a href="../">홈</a> / ${title}</div><section class="hero"><div class="eyebrow">SEULMOKAN</div><h1>${title}</h1><p class="lead">${lead}</p></section><article class="content-card">${body}${extra}<h2>운영 원칙</h2><ul><li>실제로 작동하는 도구만 공개합니다.</li><li>가능한 모든 처리는 브라우저 내부에서 수행합니다.</li><li>광고가 도구 사용을 방해하지 않도록 배치합니다.</li><li>오류와 변경된 계산 기준을 지속적으로 바로잡습니다.</li></ul><p>최종 수정일: 2026년 9월 9일</p></article></main>${footer.replaceAll('../../','../')}<script src="../assets/common.js"></script></body></html>`;const dir=path.join(root,slug);fs.mkdirSync(dir,{recursive:true});fs.writeFileSync(path.join(dir,'index.html'),completeMetadata(html));}
const urls=[base,...Object.keys(infoPages).map(x=>base+x+'/'),...tools.map(x=>base+'tools/'+x[0]+'/')],lastmod='2026-09-09';fs.writeFileSync(path.join(root,'sitemap.xml'),'<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'+urls.map(u=>`  <url><loc>${u}</loc><lastmod>${lastmod}</lastmod></url>`).join('\n')+'\n</urlset>\n');fs.writeFileSync(path.join(root,'robots.txt'),`User-agent: *\nAllow: /\nSitemap: ${base}sitemap.xml\n`);fs.writeFileSync(path.join(root,'404.html'),`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex"><title>페이지를 찾을 수 없습니다 | 쓸모칸</title><link rel="icon" href="${base}assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="${base}assets/site.css"></head><body><main class="wrap"><article class="content-card"><div class="eyebrow">404</div><h1>페이지를 찾을 수 없습니다</h1><p>주소가 변경되었거나 존재하지 않는 페이지입니다.</p><p><a class="btn" href="${base}">전체 도구로 돌아가기</a></p></article></main></body></html>`);console.log(`generated ${tools.length} tools, ${Object.keys(infoPages).length} info pages, ${urls.length} sitemap URLs`);
